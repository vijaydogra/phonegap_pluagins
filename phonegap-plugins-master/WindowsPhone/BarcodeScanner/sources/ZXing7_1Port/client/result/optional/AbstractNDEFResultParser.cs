	...
	abstract class AbstractNDEFResultParser:ResultParser
	{
		...
		internal static System.String bytesToString(sbyte[] bytes, int offset, int length, System.String encoding)
		{
			try
			{
                ...
                // tempStr = System.Text.Encoding.GetEncoding(encoding).GetString(SupportClass.ToByteArray(bytes)); // commented by .net follower (http://dotnetfollower.com)
                byte[] inputBytes = SupportClass.ToByteArray(bytes); // added by .net follower (http://dotnetfollower.com)
                tempStr = System.Text.Encoding.GetEncoding(encoding).GetString(inputBytes, 0, inputBytes.Length); // added by .net follower (http://dotnetfollower.com)
